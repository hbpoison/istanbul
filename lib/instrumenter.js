...
(function (isNode) {
    ...
    Instrumenter.prototype = {
        ...
        instrumentSync: function (code, filename) {
            ...
            var target_code = code;
            if (this.opts.traceur) {
                var traceur = require('traceur');
                var options = traceur.Compiler.commonJSOptions({ sourceMaps: true });
                var compiler = new traceur.NodeCompiler(options);
                compiler.options_.filename = filename;
                target_code = compiler.compile(code);
                this._sourceMap = new require('source-map').SourceMapConsumer(compiler.getSourceMap());
            }
            program = ESP.parse(target_code, {
                loc: true,
                range: true,
        ...
        getPreamble: function (sourceCode, emitUseStrict) {
            if (this.opts.traceur) {
                var self = this;
                var filename = this.coverage.path;
